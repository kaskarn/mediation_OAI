...
## to do: model specification, use IPW package
...
d3 %<>% mutate(obcat = cut(bmi, breaks = c(min(bmi, na.rm = T),25,30,35, max(bmi, na.rm = T))),
              depcat = cut(hsmss, breaks = c(0, 40, 50, 60, 100)),
